...
import { changeIconsClass,changeHidden } from '../features/reducer.js'
    ...
    <nav className='text-emerald-950 h-[56px] relative flex justify-between w-[90%] m-auto items-center'>
      ...
      <div>
        <ul className='hidden lg:flex gap-2 text-sm'>
          <li className='px-3 py-2 transition-all duration-1000 hover:bg-[#075985] hover:text-white rounded-sm'><Link to='/'>Ingrediants</Link></li>
          ...
          <li className='px-4 py-2 rounded-sm transition-all duration-1000 bg-[#075985] text-white hover:bg-blue-500 border-none'><Link to='/'>Add Project</Link></li>
        </ul>
        <div onClick={() => {
      ...
      <div className={`w-[150px] ${changeHiddenClass} border border-black absolute right-0 top-12 rounded lg:hidden`}>
        <ul className='lg:flex gap-2 text-sm'>
          ...
          <li className='px-4 py-2 transition-all duration-500 bg-[#075985] text-white hover:bg-blue-500 border-none'><Link to='/'>Add Project</Link></li>
        </ul>
      </div>
    </nav>
  )
